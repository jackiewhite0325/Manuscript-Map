...
function createProject(name) {
  ...
  const id = uid();
  const project = { id, name: name.trim() || "Untitled Project", createdAt: new Date().toISOString() };
...
function initRouter() {
  ...
  const valid = ["dashboard", "checklist", "metadata", "questionnaire", "assets", "arcs", "reviews"].includes(hash)
    ? hash
    : "dashboard";
  showPage(valid);
  ...
  window.addEventListener("hashchange", () => {
    ...
    if (["dashboard", "checklist", "metadata", "questionnaire", "assets", "arcs", "reviews"].includes(p)) showPage(p);
  });
}
...
const CHECKLIST_DATA = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "m2", label: "Fill Metadata Vault completely", hint: '<a href="#metadata">Open Metadata Vault →</a>' },
...
function renderArcs() {
  ...
  const filtered =
    arcsFilter === "all" ? list : list.filter((a) => a.status === arcsFilter);
